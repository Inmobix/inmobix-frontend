...
export class DashboardComponent implements OnInit {
  currentView: 'menu' | 'list' | 'create' | 'edit' | 'search' | 'profile' =
    'menu';
  loading = false;
  ...
  // Cambiar vista
  showView(
    view: 'menu' | 'list' | 'create' | 'edit' | 'search' | 'profile'
  ): void {
  ...
  loadAvailableProperties(): void {
    ...
    this.propertyService.getAvailableProperties().subscribe({
      ...
      error: (error) => {
        ...
        Swal.fire(
          'Error',
          'No se pudieron cargar las propiedades disponibles',
          'error'
        );
        this.loading = false;
      },
  ...
  updateProperty(): void {
    ...
    this.propertyService
      .updateProperty(this.editingPropertyId, this.propertyForm)
      .subscribe({
        next: (response) => {
  ...
  searchByTransaction(): void {
    if (!this.searchTransaction) {
      Swal.fire(
        'Advertencia',
        'Por favor selecciona un tipo de transacción',
        'warning'
      );
      return;
    }
    this.loading = true;
    this.propertyService
      .getPropertiesByTransaction(this.searchTransaction)
      .subscribe({
        next: (data) => {
          this.properties = data;
          this.currentView = 'list';
  ...
  searchByPriceRange(): void {
    ...
    this.loading = true;
    this.propertyService
      .getPropertiesByPriceRange(this.minPrice, this.maxPrice)
      .subscribe({
        next: (data) => {
          this.properties = data;
          this.currentView = 'list';
  ...
  buscarUsuarioPorId(): void {
    Swal.fire({
      ...
      input: 'text', // Cambiado de "number" a "text" para aceptar UUID
      inputLabel: 'Ingresa el ID del usuario',
      inputPlaceholder: 'ID del usuario (UUID)',
      showCancelButton: true,
      ...
      if (result.isConfirmed) {
        const userId = result.value; // Mantener como string
